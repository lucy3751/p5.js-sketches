...
let columns;
let rows;
let scale = 20; //how big the triangle polygons will be
...
function preload(){
    ...
    zImage = loadImage('assets/mask.png');
...
function mask(){
    ...
    //get the color of the pixels of the background in the image, and use that as a mask -  https://canvascloud.ocadu.ca/courses/2345/pages/class-recording-friday-march-19-3-10pm?module_item_id=109806
    let imageData;
    ...
    translate(-canvasWidth/2, -canvasHeight/2);
        for (var y=0; y<canvasHeight; y+=30){
            ...
            for (x=0; x<canvasWidth; x+=30){

                imageData = zImage.get(x,y);
                
                if (JSON.stringify(imageData) == JSON.stringify([0,0,0,0])){
                ...
                if (JSON.stringify(imageData) == JSON.stringify([0,0,0,255])){
...
function drawMovingTerrain(){
    ...
    push();
        ...
        translate(-canvasWidth/2 + angleChange,0);
