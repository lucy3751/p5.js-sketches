...
function draw(){
    ...
    //3D terrain generation - https://www.youtube.com/watch?v=IKB1hWWedMk&ab_channel=TheCodingTrain
        ...
        for (var y=0 ; y<rows - 1; y++){//rows - 1 because doing the array for every single row and the row below it
            ...
            beginShape(TRIANGLE_FAN); //other shapes - https://p5js.org/reference/#/p5/beginShape

            for (var x = 0 ; x<columns; x++){

                //rect(x*scale, y*scale,scale,scale);
                vertex(x*scale, (y+1)*scale, terrain[x][y]);
                vertex(x*scale, (y+1)*scale + terrain[x][y+1], terrain[x][y+1]); //y + 1 because we are drawing the top left vertex first, then the bottom left (y+1), then the top right, bottom right, so on...
